...
import { ReferenceMap, TypeHint, resolveHint, analyse } from "../src/analysis/analyse.js";
import { Resolver, TypedElement, FunctionRegistry } from "../src/analysis/resolve.js";
...
describe("resolveHint", () => {
  ...
  describe("APPLY return type assertion", () => {
    ...
    it("in #each collection context (strong parent) throws", () => {
      ...
      expect(() =>
        analyse(
          el("#each", "item in fn x", [el(null, "item")]),
          refs,
          fnResolver,
        ),
      ).toThrow();
...
let nextId = 0;
function el(keyword: string | null, expr: string, children: Element[] = []): Element {
  return {
    id: nextId++,
    keyword,
...
describe("analyse", () => {
  it("simple element binds weak string", () => {
    const refs = ReferenceMap.create();
    analyse(el(null, "name"), refs, resolver);
  ...
  it("#if binds weak boolean", () => {
    const refs = ReferenceMap.create();
    analyse(el("#if", "active", [el(null, "x")]), refs, resolver);
  ...
  it("#each binds collection from item usage", () => {
    const refs = ReferenceMap.create();
    analyse(
      el("#each", "item in items", [el(null, "item.name")]),
      refs,
      resolver,
    );
  ...
  it("#each with expression collection: a + b", () => {
    const refs = ReferenceMap.create();
    analyse(
      el("#each", "item in a + b", [el(null, "item.name")]),
      refs,
      resolver,
    );
    expect(refs.get("a")!.strong).toBe(true);
  ...
  it("#each missing in", () => {
    const refs = ReferenceMap.create();
    expect(() =>
      analyse(el("#each", "items"), refs, resolver),
  ...
  it("#each non-leaf declaration", () => {
    const refs = ReferenceMap.create();
    expect(() =>
      analyse(el("#each", "a.b in items"), refs, resolver),
